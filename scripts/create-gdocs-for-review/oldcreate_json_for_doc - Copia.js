...
function template_12(curr_node) {
    var block_output = create_default_intro_block(curr_node);
    curr_node = block_output;
    ...
    block_output = create_gender_split_block(curr_node);
    curr_node = block_output;
...
function template_13(curr_node) {
    var block_output = create_default_intro_block(curr_node);
    curr_node = block_output;
...
function template_14(curr_node) {
    var block_output = create_default_intro_block(curr_node);
    curr_node = block_output;
...
////////////////////////////////////////////
// old_media block 
function create_old_media_block(curr_node) {
    ...
    if (curr_node.hasOwnProperty('router') && curr_node.router.operand == "@fields.type_of_media") {
        if (curr_node.router.cases.length == 1) {
            if (curr_node.router.cases[0].arguments[0] == "high") {
                ...
                video["Text"] = video_node.actions[0].text;
                if (video_node.actions[0].attachments.length > 0) {
                    video["Link"] = (video_node.actions[0].attachments[0].slice(6, -2)).replace("@(fields.voiceover_video_path & \"", "https://idems-media-recorder.web.app/storage/project/PLH/subproject/Rapidpro/deployment/Global/resourceGroup/voiceover/resourceType/video/eng/")
                } else {
                    video["Link"] = "missing";
                }
    ...
    do {
        ...
        if (message.length > 0) {
            ...
            if (curr_node.hasOwnProperty('router') && curr_node.router.operand == "@fields.gender") {
                ...
                curr_node = curr_flow.nodes.filter(function (nd) { return (nd.uuid == default_node.exits[0].destination_uuid) })[0];
            } else {
                go_on = false;
                console.log("this is not a send message node")
                next_node = null;
...
/////////////////////////////////////////////////////////////////////
// gender splits
function create_gender_split_block(curr_node) {
